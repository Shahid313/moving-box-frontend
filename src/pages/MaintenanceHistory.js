...
import '../css/MaintenanceHistory.css';
...
function MaintenanceHistory() {
  ...
  const getMaintenanceHistory = () => {
    axios.get('http://localhost:5000/apis/maintenance_history/get').then((response) => {
  ...
  const handleChangeSearch = (e) => {
    console.log(e)
    const search_result = []
    maintenanceHistory.map((mhistory) => {
      let result2 = new RegExp(e, 'i').test(mhistory.matricola)
      if(result2 == true){
        search_result.push(mhistory)
      }
    })
  ...
  return (
    ...
    <h2 className='data__title'>Maintenace history</h2>
      <div className='search__input__field__box'>
      <input onChange={(e) => handleChangeSearch(e.target.value)} className='search__input' type='text' placeholder='Search'/>
      </div>
    <table className="fl-table">
        ...
        <tbody>
        {searched_results == "" ?
        maintenanceHistory.map((mhistory, index) => (
          <tr index={index}>
          <td className='tableRow'>{mhistory.date}</td>
          <td className='tableRow'>{mhistory.maintainer}</td>
          <td className='tableRow'>{mhistory.matricola}</td>
          <td className='tableRow'>{mhistory.modello}</td>
          <td className='tableRow'>{mhistory.ubicazione}</td>
          <td className='tableRow'>{mhistory.intervento}</td>
          <td className='tableRow'>
            {mhistory.intervento === 'positive' ?
            <button className='positive_outcome__btn'>Positive</button>:
            <button className='negative_outcome__btn'>Negative</button>
            }
          </td>
          <td className='tableRow'>{mhistory.company_name}</td>
          <td className='tableRow'>
          <Link to={`http://localhost:5000/uploads/reports/${mhistory.download}`} download>
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="#03a9f4" style={{height:18, width:18}}>
        ...
        searched_results.map((mhistory, index) => (
          <tr index={index}>
          <td className='tableRow'>{mhistory.date}</td>
          <td className='tableRow'>{mhistory.maintainer}</td>
          <td className='tableRow'>{mhistory.matricola}</td>
