...
function AddTokens() {
  ...
  const add_token = (e) => {
    ...
    if(quantity === "" || price === ""){
      ...
      axios.post('http://localhost:5000/apis/tokens/add', data).then((response) => {
  ...
  return (
    ...
    <h2 className='data__title'>Add token</h2>
    <div className='add_company_fields'>
        <input type='text' onChange={(e) => setQuantity(e.target.value)} placeholder='Enter quantity'/>
        <input type='text' onChange={(e) => setPrice(e.target.value)} placeholder='Enter price'/>

        <button onClick={(e) => add_token(e)} className='add_company_btn'>Add Token</button>
    </div>
    </div>
