...
function AddCompany() {
  ...
  const addCompany = (e) => {
    ...
    if(companyName === "" || address === "" || city === "" || postalCode === "" || vatNumber === "" || nation === ""){
      ...
      data.append('vat_number',vatNumber)
      data.append('nation',nation)
      axios.post('http://localhost:5000/apis/company/add', data).then((response) => {
  ...
  return (
        ...
        <div className='Nav'>
            ...
            <div className={showHamburger ? 'mobile__nav': 'mobile__nav__not__shown'}>
              <ul className={showHamburger ? 'monile__navigation__links__shown': 'monile__navigation__links_not_shown'}>
                ...
                <li><Link to='/admin/maintenance/history'>Maintenance history</Link></li>
                <li><Link to='/admin/token/add'>Add token</Link></li>
                <li><button onClick={(e) => logoutUser(e)} className='mobile__logout__btn'>Logout</button></li>
              </ul>
            </div>
        </div>
    ...
    <h2 className='data__title'>Add company</h2>
    <form className='add_company_fields'>
        ...
        <input type='text' onChange={(e) => setPostalCode(e.target.value)} placeholder='Postal code'/>
        <input type='text' onChange={(e) => setVatNumber(e.target.value)} placeholder='Vat number'/>
        <input type='text' onChange={(e) => setNation(e.target.value)} placeholder='Nation'/>
        <button onClick={(e) => addCompany(e)} className='add_company_btn'>Add company</button>
    </form>
    </div>
