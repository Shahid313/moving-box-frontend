...
import '../../css/AddMaintenanceHistory.css';
...
function AddMaintenanceHistory() {
  ...
  const add_maintenance_history = (e) => {
    e.preventDefault()
    ...
    if(outcome === ""){
      ...
      alert("Please select an intervento")
      return;
    }else if(companyId === ""){
      alert("Please select a company")
      return;
    }else if(date === "" || maintainer === "" || matricola === "" || modello == "" || ubicazione === ""){
      alert("Please fill all the necessary fields")
    }else{
      const data = new FormData()
      data.append('date',date)
      data.append('maintainer',maintainer)
      data.append('matricola',matricola)
      data.append('modello',modello)
      data.append('ubicazione',ubicazione)
      data.append('intervento',intervento)
      data.append('outcome',outcome)
      data.append('download',pdfFile)
      data.append('companyId',companyId)

    axios.post('http://localhost:5000/apis/maintenance_history/add', data).then((response) => {
            if(response.data.status === 'created'){
                alert("Maintenance history added")
            }else{
  ...
  return (
    ...
    <h2 className='data__title'>Add Maintenance History</h2>
    <div className='add_company_fields'>
        <input type='text' onChange={(e) => setDate(e.target.value)} placeholder='Date'/>
        <input type='text' onChange={(e) => setMaintainer(e.target.value)} placeholder='Maintainer'/>
        <input type='text' onChange={(e) => setMatricola(e.target.value)} placeholder='Matricola'/>
        <input type='text' onChange={(e) => setModello(e.target.value)} placeholder='Modello'/>
        <input type='text' onChange={(e) => setUbicazione(e.target.value)} placeholder='Ubicazione'/>

        <select className='companies_names' onChange={(e) => setIntervento(e.target.value)}>
            <option>Select intervento</option>
            <option value="INTERVENTO">INTERVENTO</option>
            <option value="MANUTENZIONE">MANUTENZIONE</option>
        </select>


        <select className='companies_names' onChange={(e) => setOutcome(e.target.value)}>
        ...
        <label className='add_report_btn' for="report">Add report</label>
        <input style={{display:'none'}} id="report" type='file' onChange={(e) => setPdfFile(e.target.files[0])}/>
        <button onClick={(e) => add_maintenance_history(e)} className='add_company_btn'>Add</button>
        </div>
        </div>
...
export default AddMaintenanceHistory;